# ==============================================================================
# POWERBALL PRODUCTION SALES & TICKET PREDICTOR (v1.0)
...
target_draw_date <- as.Date("2026-06-27") # Upcoming draw date (Saturday)
target_jackpot   <- 80000000              # Estimated Jackpot amount in dollars
...
load_db_data <- function(cfg) {
  ...
  pb <- pb %>%
    mutate(
      Is_Reset_A = as.integer(Jackpot <= 20e6), # Powerball starts at $20M
...
generate_prediction <- function(pb_data, target_date, target_jp, cfg) {
  ...
  if (prev_jp >= 0.5 * target_jp && target_jp <= 20e6) {
    is_reset_final <- 1
    roll_count <- 1
  }
  ...
  # Define test row
  test_row <- data.frame(
    DrawDate = target_date, Jackpot = target_jp,
  ...
  train_data <- pb_data %>% filter(DrawDate < target_date)
  ...
  # 9. Find Historical Analogues (Same Day of Week, Jackpot +/- 20%)
  analogues <- pb_data %>%
    filter(
      DayOfWeek == dow,
...
pb_data <- load_db_data(cfg)
pred <- generate_prediction(pb_data, target_draw_date, target_jackpot, cfg)

# Print Detailed Predictor Report
cat("\n")
cat(strrep("=", 80), "\n")
cat("            POWERBALL SALES & TICKET PREDICT REPORT\n")
cat(strrep("=", 80), "\n\n")
...
cat(sprintf("  Ticket Price:       $%d (Fixed)\n", pred$price))
...
cat(strrep("-", 80), "\n")
cat("  HISTORICAL ANALOGUES (Same Day of Week, Jackpot +/-20%)\n")
cat(strrep("-", 80), "\n")
...
if (nrow(pred$analogues) > 0) {
  ...
  cat("   No similar historical jackpots found on this day.\n")
}
cat("\n")
...
), file.path(out_dir, "upcoming_pb_draw_prediction.csv"), row.names = FALSE)
