# ==============================================================================
# MEGA MILLIONS RECENCY MOMENTUM VALIDATION & BACKTEST
...
align_levels <- function(model, newdata, default_day = "Tue") {
...
pred_M7_ConstrainedGAM <- function(model, newdata) {
  ...
  newdata <- align_levels(model$fit, newdata, default_day = "Tue")
...
load_data <- function(cfg) {
  ...
  mm <- df_raw %>%
    ...
    mutate(
      ...
      JP_Band = assign_mm_band(Jackpot)
    )
...
get_lookahead_free_pred <- function(train_subset, rd) {
  if (rd$Is_Reset_Final[1] == 1) {
    ...
    if (!is.null(m_res)) {
      rd_aligned <- align_levels(m_res, rd, default_day = "Tue")
...
run_momentum_backtest <- function(mm_data, cfg) {
  ...
  # Fit models periodically to save time (refit_every)
  model_base <- NULL
  model_reset <- NULL
  sm_reset <- 1.0
  r_mean <- 0
  ...
  for (i in (min_train + 1):n) {
    ...
    if (eval_row$Is_Reset_Final[1] == 1) {
      if (!is.null(model_reset)) {
        eval_row_aligned <- align_levels(model_reset, eval_row, default_day = "Tue")
    ...
    # Multipliers for different schemes
    mult_A <- 1.0 # 50/30/20
    mult_B <- 1.0 # 70/20/10
...
mm_data <- load_data(cfg)
res <- run_momentum_backtest(mm_data, cfg)
scorecard <- compile_scorecard(res)

# Print Summary Console Report
cat("\n")
cat(strrep("=", 110), "\n")
cat("          MEGA MILLIONS RECENCY MOMENTUM AUDIT — VALIDATION RESULTS\n")
cat(strrep("=", 110), "\n\n")
...
write.csv(res, file.path(cfg$OUTPUT_DIR, "momentum_backtest_detail.csv"), row.names = FALSE)
write.csv(scorecard, file.path(cfg$OUTPUT_DIR, "momentum_scorecard.csv"), row.names = FALSE)

message(">>> Momentum validation complete! Outputs exported to: ", cfg$OUTPUT_DIR)
