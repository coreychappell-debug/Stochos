...
function handleCenterPanelChange(e) {
    ...
    if (['units', 'vendorId', 'ticketSize'].includes(prop)) {
        // Redraw center panel totals that might have changed dynamically immediately
        renderCenterPanel();
    }
}
...
function handleRightPanelClick(e) {
    const target = e.target.closest('[data-vendor-id], #manage-features-btn, [data-action="toggle-right"], [data-action="add-tier"], [data-action="delete-tier"]');
    if (!target) return;
    ...
    if (target.dataset.action === 'add-tier') {
        const vId = target.dataset.vendor;
        const sizeId = target.dataset.size;
        commit(s => {
            const vendorOptions = s.vendorPricing[vId].baseCosts;
            if (!vendorOptions[sizeId]) vendorOptions[sizeId] = [];

            let defaultQty = 1000000;
            const existingTiers = vendorOptions[sizeId];
            if (existingTiers.length > 0) {
                // Pick a quantity higher than the highest to be helpful
                defaultQty = Math.max(...existingTiers.map(t => t.quantity)) + 1000000;
            }

            vendorOptions[sizeId].push({ quantity: defaultQty, cost: 0.00 });
            vendorOptions[sizeId].sort((a, b) => a.quantity - b.quantity);
        }, `Added pricing tier to ${vId} ${sizeId}.`);
        // We know we must re-evaluate all games to see if their base cost shifted
        renderCenterPanel();
        return;
    }
    ...
    if (target.dataset.action === 'delete-tier') {
        ...
        commit(s => {
            const tiers = s.vendorPricing[vId].baseCosts[sizeId];
            if (tiers && index >= 0 && index < tiers.length) {
                tiers.splice(index, 1);
            }
        }, `Deleted pricing tier from ${vId} ${sizeId}.`);
        renderCenterPanel();
        return;
    }
    ...
    } else if (target.dataset.vendorId && !target.dataset.action) {
...
function handleRightPanelChange(e) {
    ...
    if (e.target.name === `costModel-${vendorId}`) {
        ...
        renderCenterPanel();
    } else if (e.target.classList.contains('tier-qty-input') || e.target.classList.contains('tier-cost-input')) {
        const vId = e.target.dataset.vendor;
        const sizeId = e.target.dataset.size;
        const index = parseInt(e.target.dataset.index, 10);

        let value = e.target.value;
        if (e.target.classList.contains('tier-qty-input')) {
            value = parseFloat(value.replace(/,/g, '')) || 0;
            e.target.value = value.toLocaleString('en-US'); // format immediately
        } else {
            value = parseFloat(value) || 0;
        }

        commit(s => {
            const tier = s.vendorPricing[vId].baseCosts[sizeId][index];
            if (tier) {
                if (e.target.classList.contains('tier-qty-input')) tier.quantity = value;
                else tier.cost = value;
            }
        }, `Updated ${vId} ${sizeId} pricing tier.`);
        // Re-calculate math in center panel since base costs shifted
        renderCenterPanel();
    } else {
