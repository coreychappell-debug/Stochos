...
*   **Instant Ticket Planner & Budgetary Tool:** An interactive, local-first PWA for modeling scratcher game portfolios and planogram compliance. Key capabilities include:
    *   **Interactive "What-If" Financial Modeler:** Real-time simulation of financial scenarios. Drag range sliders to adjust sell-through rates (50% to 100%), retailer commissions (1% to 15%), print run scaling (-50% to +50%), and return rate offsets (-15% to +15%). See calculations scale printed units, adjust returns offsets, compute step-tier printing costs, and dynamically re-weight central overhead share instantly.
    *   **Prior-Year Carryover & Continuous Portfolio Seeding:** Eliminates manual data entry when transitioning between fiscal planning cycles. The tool automatically clones active scratcher games from the previous year, initializing them with an `already_booked` budget status to prevent double-counting manufacturing print costs while keeping gross sales and lifecycles continuous.
    *   **Universal Retail Mix Planner & Planogram Compliance Engine:** Models physical dispenser layouts (24-Bin, 30-Bin, 40-Bin Default, and 48-Bin profiles) against active game inventory. Features an age-coded visual grid (Fresh, Active/Established, Double-Faced, Empty slots) and a real-time compliance scoring system.
    *   **Double-Facing & Pack Splitting Compliance:** Standardize double-facing in empty bins to satisfy target mix requirements, fully complying with lottery security policies that prohibit physical pack splitting in the field.
    *   **Visual Ticket Swatch Previews:** Renders dynamic ticket thumbnails using preloaded color themes or custom-uploaded ticket art, displaying swatches directly in the scheduler and compliance grids.
    *   **Product Availability & Procurement Pipeline Trackers:** Displays real-time counts of actively available games selling in the field vs. games in the procurement pipeline (ordered, in production, shipped, received) to check inventory health at a glance.
    *   **Operator Ergonomics & Exports:** Sort Gantt charts by price point or launch month, collapse sections for focused numbers reviews, pin default view preferences in browser storage, export planning rosters to CSV (including calculated lifespans and close dates), and print reports with optimized stylesheets that automatically hide interactive controls.
