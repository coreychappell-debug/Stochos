...
import { CONTRACT_UNIT_PRESETS, STANDARD_DENOMINATIONS, STANDARD_TICKET_SIZES } from './constants.js';
...
export function renderRightPanel() {
    ...
    let content = vendors.map(v => {
        ...
        const baseCostRows = STANDARD_TICKET_SIZES.map(size => {
            ...
            const tierRows = tiers.map((t, idx) => `
                <tr style="background: rgba(255,255,255,0.02);">
                    <td style="padding-left: 0.75rem;">Qty ≥ <input type="text" class="tier-qty-input" value="${t.quantity.toLocaleString('en-US')}" data-vendor="${v.id}" data-size="${size.id}" data-index="${idx}" style="width:90px; display:inline-block; font-size: 0.8em; padding: 2px;"></td>
                    <td>Cost: $<input type="number" class="tier-cost-input" value="${t.cost}" step="0.001" data-vendor="${v.id}" data-size="${size.id}" data-index="${idx}" style="width:80px; display:inline-block; font-size: 0.8em; padding: 2px;"></td>
                    <td style="text-align: right;"><button data-action="delete-tier" data-vendor="${v.id}" data-size="${size.id}" data-index="${idx}" style="font-size: 0.75rem; color: var(--danger-color); padding: 2px 6px;">🗑️</button></td>
                </tr>
            `).join('');
        ...
        const featureRows = state.featureMasterList.map(f => {
            ...
            return `<tr><td style="padding: 0.25rem 0;" colspan="2">${name}</td><td style="text-align:right;"><input type="number" value="${saved}" placeholder="0.00" step="0.01" data-feature-name="${name}" style="width: 80px;"></td></tr>`;
        }).join('');
        ...
        return `
            <div class="tab-content" style="margin-top: 1rem; ${state.ui.activeVendorTab === v.id ? 'display:block;' : ''}" id="tab-${v.id}">
                ...
                <table style="width: 100%; font-size: 0.875rem; border-collapse: collapse;">
                    <tbody>
                        <tr><td colspan="3" style="font-weight: 600; padding: 0.5rem 0 0.25rem 0; background: var(--bg-light); border-bottom: 1px solid #4b5563;">Linear Interpolation Tiers (Per Thousand)</td></tr>
                        ${baseCostRows}
