...
export default function CrmImportClient({ routes = [], chains = [] }) {
  ...
  const handleDownloadPrepopulatedTemplate = () => {
    const url = `/api/crm/retailers/export?templateType=${domain}&routeId=${exportRouteId}&chainId=${exportChainId}&status=${exportStatus}`;
  ...
  const handleImportSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/crm/import?domain=${domain}&dryRun=${dryRun}`, {
        method: "POST",
