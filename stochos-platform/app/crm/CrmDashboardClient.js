...
export default function CrmDashboardClient({ stats, recentVisits, openExceptions, routes }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Link href="/crm/import" className="btn btn-secondary">
            📥 Importer Console
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(450px, 1fr))", gap: 24 }}>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {openExceptions.map((ex) => (
                    <tr key={ex.id}>
                      <td>
                        <Link href={`/crm/retailers/${ex.retailerId}`} style={{ fontWeight: 600 }}>
                          {ex.retailer?.name}
                        </Link>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <Link href="/crm/retailers" style={{ fontSize: 12 }}>View Registry</Link>
          </div>
          <div className="card-body" style={{ padding: 0 }}>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {recentVisits.map((v) => (
                    <tr key={v.id}>
                      <td>
                        <Link href={`/crm/retailers/${v.retailerId}`} style={{ fontWeight: 600 }}>
                          {v.retailer?.name}
                        </Link>
                      </td>
                      <td>{v.user?.name}</td>
                      ...
                      <td>
                        <Link href={`/crm/visits/${v.id}`} className="btn btn-secondary btn-sm">
                          Details
                        </Link>
                      </td>
                    </tr>
