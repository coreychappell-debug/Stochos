...
export default function CrmRetailerDetailClient({ retailer }) {
  ...
  const handleResolveException = async (excId) => {
    ...
    setResolving(excId);
    try {
      const res = await fetch(`/api/crm/exceptions/${excId}/resolve`, {
        method: "POST"
      });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Link href={`/crm/visits/new?retailerId=${retailer.id}`} className="btn btn-primary">
            📝 Log Store Visit
          </Link>
        </div>
      </div>
