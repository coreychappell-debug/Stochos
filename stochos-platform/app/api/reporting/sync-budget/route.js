...
export async function POST(request) {
  ...
  try {
    ...
    for (const prop of approvedProposals) {
      ...
      divisionBudgets[`total_divisional_budget_${prop.division.toLowerCase()}`] = sum;
    ...
    if (activePlan && activePlan.scenarios.length > 0) {
      ...
      for (const game of scenario.games) {
        ...
        // Estimate print cost (we'll query vendor pricing or approximate based on units)
        // SG is percent_of_sales, Pollard/IGT are per_thousand
        let printCost = 0;
        ...
        if (vendorId) {
          ...
          if (pricing) {
            ...
            // Fallback default printing cost: $22 per thousand
      ...
      // Calculate Scratcher marketing overhead
    ...
    console.error("Failed to compile budget rollup:", err);
