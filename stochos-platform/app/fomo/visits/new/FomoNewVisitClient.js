...
export default function FomoNewVisitClient({ retailers, initialRetailer }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/fomo/retailers/${selectedRetailerId}`)
      .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/fomo/visits", {
        method: "POST",
      ...
      router.push(`/fomo/retailers/${selectedRetailerId}`);
      router.refresh();
    } catch (err) {
